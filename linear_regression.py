...
# Ignore warning generated by statsmodels module
...
def parse_args():
    ...
    parser.add_argument("-vv", '--very-verbose', dest='vverbose', action='store_true',
                        help="Print verbose information during execution.")
    parser.add_argument('--no-very-verbose', dest='vverbose', action='store_false',
                        help="Default: does not print verbose information during execution.")
    parser.set_defaults(plot=False, verbose=False, vverbose=False)
...
def plot_fit(fits, ax):
    lines_colors = []
    for fit in fits:
        results = maybe_unwrap_results(fit)
        _, exog_idx = maybe_name_or_idx(0, fit.model)
        try:
            if len(results.model.exog[0]) > 1:
                _, exog_idx = maybe_name_or_idx(1, fit.model)
        except TypeError:
            pass
        ...
        line = ax.plot(x1, results.fittedvalues[x1_argsort], '--', label='fitted', linewidth=2.5)
        lines_colors.append(next(iter(line or []), None)._color)
    lines_colors.append('r')

    return lines_colors


def plot_prediction(ax, predictions, lines_colors):
    ax_plot = ft.partial(ax.plot, marker='d')
    ax_annotate = ft.partial(ax.annotate, textcoords='data')

    for i, ((date, prediction, name), color) in enumerate(zip(predictions, lines_colors)):
        xy = (date, prediction)
        ax_plot(*xy, color=color)
        ax_annotate(xy=xy, s='Prediction %d' % (i + 1) if name is None else name)


def plot_model_and_prediction(df, fits, date, *predictions):
    # Plot data
    ax = df.plot(x='CheckCashed', y='Approved', kind='scatter')

    # Make sure blue isn't reused
    ax.set_prop_cycle(cycler('color', ['c', 'm', 'y', 'g']))

    # Plot fit
    lines_colors = plot_fit(fits, ax)

    # Plot prediction
    plot_prediction(ax, predictions, lines_colors)
...
def main():
    ...
    if args.verbose or args.vverbose:
        for result, prediction in zip(results, tuple(map(np.vectorize(convert_back_to_datetime), predictions))):
            if args.vverbose: print result.summary()
    ...
    if args.plot:
        ...
        predictions = zip([date] * length, predictions, [None] * length)
        predictions.append((date, prediction, 'Mean Prediction'))
        plot_model_and_prediction(df, results, date, *predictions)
