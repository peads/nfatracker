...
trait Regression {
  ...
  protected def generateData(url: String): (Array[String],
    ...
    // The settings object provides many configuration options// The settings object provides many
    // configuration options
    val parserSettings = new CsvParserSettings

    //You can configure the parser to automatically detect what line separator sequence is in the
    // input
    ...
    // You will find more RowProcessors in the 'com.univocity.parsers.common.processor' package,
    ...
    // the 'parse' method will parse the file and delegate each parsed row to the RowProcessor you
    // defined
    parser.parse(reader)

    // get the parsed records from the RowListProcessor here.
    // Note that different implementations of RowProcessor will provide different sets of
    // functionalities.
    (rowProcessor.getHeaders, JavaConverters.asScalaBuffer(rowProcessor.getRows).toList)
  }

  protected def filterData(headers: Array[String], rows: List[Array[String]]):
    List[(String, String, DateTime, DateTime)] = {
